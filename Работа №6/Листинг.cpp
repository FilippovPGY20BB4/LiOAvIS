...
void main()
{
	...
	while (1)
	{
		printf_s("Ââåäèòå êîëè÷åñòâî âåðøèí, -1 äëÿ çàâåðøåíèÿ ");
		scanf_s("%d", &a);
		if (a == -1) break;
		srand(time(NULL));
		...
		printf_s("Âðåìÿ ïîèñêà â øèðèíó ïî ìàòðèöå: %lf\n", time_bfs_matr);
		//
		diff_time = clock();
		BFS_spisok(start, a);
		time_bfs_sp = (clock() - diff_time) / 1000;
		printf_s("Âðåìÿ ïîèñêà â øèðèíó ïî ñïèñêó ñìåæíîñòåé: %lf\n", time_bfs_matr);
		//
		diff_time = clock();
		DFS(matrix, start, a);
		time_dfs_matr = (clock() - diff_time) / 1000;
		printf_s("Âðåìÿ ïîèñêà â ãëóáèíó ïî ìàòðèöå: %lf\n", time_dfs_matr);
		//
		...
		printf_s("Âðåìÿ ïîèñêà â ãëóáèíó ïî ñïèñêó: %lf\n", time_dfs_sp);
		delete_spisok(a);
		free(distanse_DFS);
		for (i = 0; i < a; i++)
		{
...
void BFC_with_distanse(int** matrix, int start,int rasm)
{
	...
	while (!Q.empty())
	{
		visited = Q.front();
		Q.pop();
		printf_s("Âåðøèíà: %d, ðàññòîÿíèå: %d\n", visited,distanse[visited]);
		for (i = 0; i < rasm; i++)
		{
	...
	{
		if (distanse[i] == -1)
			printf_s("\nÂåðøèíà %d íå ïîñåùåíà, ò.ê. èçîëèðîâàíà.\n", i);
	}
	free(distanse);
}
